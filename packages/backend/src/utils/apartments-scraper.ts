...
export async function parseListingHTML(page: Page): Promise<ApartmentInput[]> {
  ...
  const rawResults = await listingsSelector?.evaluate((container) => {
    ...
    return [...listings].reduce((acc, listing) => {
      ...
      const parsedBedBath = listing.querySelector('.property-beds')?.textContent.trim() ||
  ...
  const apartmentModels = rawResults.map((l: ApartmentInput) => {
    ...
    let [beds = 0, baths = 0] = l.parsedBedBath?.split(',').map((s: string | undefined) => Number(s?.trim()?.split(' ')[0])) || [];
